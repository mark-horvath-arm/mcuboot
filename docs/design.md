...
```c
int boot_image_exec_ram_info_get(uint32_t image_id,
...
direct-xp strategy is chosen. The flash map is very similar to the swap
...
+  Loop 1. Iterate over all images
    + Does any of the slots contain an image?
        + Yes:
            ...
            + Copy it to RAM in case of ram-load stategy.
...
+  Loop 2. Iterate over all images
    + Does the current image depend on other image(s)?
        + Yes: Are all the image dependencies satisfied?
            + Yes: Skip to next image.
            + No:
                + Delete the image from RAM in case of ram-load stategy, but
