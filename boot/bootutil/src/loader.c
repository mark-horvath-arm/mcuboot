                       ...
                       struct slot_usage_t slot_usage[])
{
    ...
    if (hdr->ih_flags & IMAGE_F_ROM_FIXED && hdr->ih_load_addr != f_off) {
        BOOT_LOG_WRN("Image in %s slot at 0x%x has been built for offset 0x%x"\
                     ", skipping", active_slot == 0 ? "primary" : "secondary", f_off,
                     hdr->ih_load_addr);
 ...
 */
static int
open_flash_areas_and_get_slot_usage(struct boot_loader_state *state,
                                    struct slot_usage_t slot_usage[])
{
    uint32_t slot;
    int fa_id;
    ...
    IMAGES_ITER(BOOT_CURR_IMG(state)) {
        ...
        for (slot = 0; slot < BOOT_NUM_SLOTS; slot++) {
            ...
            rc = flash_area_open(fa_id, &BOOT_IMG_AREA(state, slot));
            if(rc != 0) {
                BOOT_LOG_ERR("Panic! Failed to open flash area.");
                while(true);
            }
        }
                             ...
                             struct slot_usage_t slot_usage[])
{
...
#ifdef MULTIPLE_EXECUTABLE_RAM_REGIONS
    rc = boot_image_exec_ram_info_get(BOOT_CURR_IMG(state), &exec_ram_start,
                                      &exec_ram_size);
    if (rc != 0) {
    ...
    if (img_end_addr > (exec_ram_start +
                        exec_ram_size)) {
        return BOOT_EBADIMAGE;
    }
...
#if (BOOT_IMAGE_NUMBER > 1)
/**
 * Checks if two memory regions (A and B) are colliding or not.
 *
 * @param  startA   Start of the A region.
 * @param  endA     End of the A region.
 * @param  startB   Start of the B region.
 * @param  endB     End of the B region.
 *
 * @return          false if there is no collision; true otherwise.
 */
static bool
do_regions_collide(uint32_t startA, uint32_t endA,
...
/**
 * Checks if the image we want to load to memory collides with an already
 * ramload image.
 *
 * @param  slot_usage         Information about the active and available slots.
 * @param  image_id_to_check  The ID of the image we would like to load.
 *
 * @return                    0 if there is no collision; nonzero otherwise.
 */
static int
boot_check_ram_load_collision(struct slot_usage_t slot_usage[],
                              uint32_t image_id_to_check)
{
    ...
    startA = slot_usage[image_id_to_check].img_dst;
    /* Safe to add here, values are already verified */
    endA = startA + slot_usage[image_id_to_check].img_sz;
    ...
    for (i = 0; i < BOOT_IMAGE_NUMBER; i++) {
        ...
        startB = slot_usage[i].img_dst;
        endB = startB + slot_usage[i].img_sz;

        if(do_regions_collide(startA, endA, startB, endB)) {
            return -1;
        }
    }
                        ...
                        struct slot_usage_t slot_usage[])
{
    ...
    if (hdr->ih_flags & IMAGE_F_RAM_LOAD) {
        ...
        rc = boot_read_image_size(state, active_slot, &img_sz); if (rc != 0) {
            return rc;
        }
...
#if (BOOT_IMAGE_NUMBER > 1)
        rc = boot_check_ram_load_collision(slot_usage, BOOT_CURR_IMG(state));
        if (rc != 0) {
            BOOT_LOG_INF("Image RAM loading to address 0x%x would collide with\
...
/**
 * Removes an image from flash by erasing the corresponding falsh area
 *
 * @param  state    Boot loader status information.
 ...
 */
static int
boot_verify_all_image_dependencies(struct boot_loader_state *state,
                                   struct slot_usage_t slot_usage[])
{
    int rc = -1;
    uint32_t active_slot;
    ...
    IMAGES_ITER(BOOT_CURR_IMG(state)) {
        ...
        if (rc != 0) {
            ...
            active_slot = slot_usage[BOOT_CURR_IMG(state)].active_slot;
            slot_usage[BOOT_CURR_IMG(state)].slot_available[active_slot]
                                                                    = false;
            slot_usage[BOOT_CURR_IMG(state)].active_slot = NO_ACTIVE_SLOT;
                              ...
                              struct slot_usage_t slot_usage[])
{
    ...
    defined(MCUBOOT_MEASURED_BOOT) || \
    defined(MCUBOOT_DATA_SHARING)
/**
 * Updates the security counter, saves boot status and saves shared data for
 * all the images.
 *
 * @param  state        Boot loader status information.
 * @param  slot_usage   Information about the active and available slots.
 ...
 */
static int
boot_hwprot_measuredboot_datasharing(struct boot_loader_state *state,
                                     struct slot_usage_t slot_usage[])
{
    uint32_t active_slot;
    int rc;
...
context_boot_go(struct boot_loader_state *state, struct boot_rsp *rsp)
{
    ...
    rc = open_flash_areas_and_get_slot_usage(state, slot_usage);
    if (rc != 0) {
        goto out;
    }
    ...
    while(true) {
#endif
        rc = boot_load_and_validate_images(state, slot_usage);
        if (rc != 0){
            goto out;
        }

#if (BOOT_IMAGE_NUMBER > 1)
        rc = boot_verify_all_image_dependencies(state, slot_usage);
        if (rc != 0) {
    ...
    defined(MCUBOOT_MEASURED_BOOT) || \
    defined(MCUBOOT_DATA_SHARING)
    rc = boot_hwprot_measuredboot_datasharing(state, slot_usage);
    if (rc != 0) {
        goto out;
    }
#endif
